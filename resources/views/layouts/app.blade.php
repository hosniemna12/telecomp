...
<body class="bg-slate-900 min-h-screen flex">
    ...
    <aside class="w-64 bg-slate-800 border-r border-slate-700 min-h-screen flex flex-col fixed left-0 top-0 z-30">
        ...
        {{-- Navigation (requires authenticated user via middleware) --}}
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
            ...
            <div class="px-3 py-2 mt-2">
                <p class="text-slate-600 text-xs uppercase tracking-wider">Rejets</p>
            </div>
            ...
            <a href="{{ route('rejets.pacs004') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition {{ request()->routeIs('rejets.pacs004') ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-700 hover:text-white' }}">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
                Pacs.004 — Rejets
                @php $nbPacs = \App\Models\TcPacs004::where('statut', 'GENERE')->count(); @endphp
                @if($nbPacs > 0)
                    <span class="ml-auto bg-orange-500 text-white text-xs px-1.5 py-0.5 rounded-full">
                        {{ $nbPacs }}
                    </span>
                @endif
            </a>
        ...
        <div class="px-3 py-4 border-t border-slate-700">
            @php
                $roleClasses = match(auth()->user()->role) {
                    'admin' => 'bg-blue-600/30 text-blue-400',
                    'operateur' => 'bg-green-600/30 text-green-400',
                    default => 'bg-purple-600/30 text-purple-400'
                };
            @endphp
            <a href="{{ route('profile.index') }}"
               class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition {{ request()->routeIs('profile.index') ? 'bg-slate-700 text-white' : 'text-slate-400 hover:bg-slate-700 hover:text-white' }}">
                <div class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0 {{ $roleClasses }}">
    ...
    <div class="flex-1 ml-64 flex flex-col min-h-screen">
        ...
        <header class="bg-slate-800 border-b border-slate-700 px-8 py-4 flex items-center justify-between sticky top-0 z-20">
            ...
            <div class="flex items-center gap-3">
                <div wire:loading class="animate-pulse inline-block px-3 py-1 bg-slate-700 rounded-md text-xs text-slate-400"></div>
        ...
        @if(session('erreur_acces'))
            <div class="mx-8 mt-6 bg-red-500/10 border border-red-500/30 text-red-400 rounded-xl px-4 py-3 flex items-center gap-2 text-sm" role="alert">
